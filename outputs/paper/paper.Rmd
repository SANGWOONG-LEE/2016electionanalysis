...
```
* number of survey participants of age in the range of 20~40 is unexpectedly low -> not many people in ages between 20-40 participated in the survey
* Clinton's vote is higher than trump's vote in this survey, which contradicts the election result of 2016, reader should keep in mind that this survey is more biased towards clinton.
...
As I mentioned above, deciding who to vote is followed by very complex system of thoughts. Thus, as we cut down the variables, they do not guarantee 100% of the vote. Also, because we did not exclude non-response from our dataset, for some graphs it caused difficulties in interpretation.
There exists similar survey similar to ours, CNN exit polls, however it is not obtain the datasets they used.
Now, let's get into the variables in details. One thing that we foucsed was whether respondents were equivalently recruited. To do this, we first have to look at respondents' personal information. If you take a look at fig(), there is not much to notice. Few things to notice are there are more female respondents than male, and for race, there are more White than others. However, following US demographics, White 76.3%, Black: 13.4%, American Indian: 1.3%, Asian: 5.9%, Hispanic: 18.5%, our sample is fairly well recruited. 
...
reduced_data %>%
  ...
  labs(x = element_blank(),
       ...
       caption = "Figure 3: Distribution of survey respondents' gender") +
  theme(legend.position = "none")
```
...
reduced_data %>%
  ...
  labs(x = element_blank(),
       ...
       caption = "Figure 4: Distribution of survey respondents' vote choice") +
  theme(legend.position = "none")
```
...
reduced_data %>%
  ...
  labs(x = element_blank(),
       ...
       caption = "Figure 5: Opinion on national spending on improving conditions of black population") +
  facet_wrap(~ pres16) + 
...
reduced_data %>%
  ...
  labs(x = element_blank(),
       ...
       caption = "Figure 6: Distribution of survey respondents' vote choice by birth place",
       fil = "Candidate")
```  
...
reduced_data %>%
  ...
  labs(x = element_blank(),
       ...
       caption = "Figure 7: Distribution of survey respondents' vote by gender",)
```
...
reduced_data %>%
  ...
  labs(x = element_blank(),
       ...
       caption = "Figure 8: Opinion on national spending on welfare") +
  facet_wrap(~ pres16) + 
  theme(legend.title = element_blank())
```
...
reduced_data %>%
  ...
  labs(x = element_blank(),
       ...
       caption = "Figure 9: Distribution of votes by degree and family income")
```
...
reduced_data %>%
  ...
  labs(x = element_blank(),
       ...
       caption = "Figure 10: Opinion on wheter the US should limit immigration to protect their culture") +
  facet_wrap(~pres16)
```
...
reduced_data %>%
  ...
  labs(x = element_blank(),
       ...
       caption = "Figure 11: Surveyee's opinion on government's budget on education improvement")
```
...
reduced_data %>%
  ...
  labs(x = element_blank(),
       ...
       caption = "Figure 12: Surveyee's opinion on government's budget on arms")
```
...
reduced_data %>%
  ...
  labs(x = element_blank(),
       ...
       caption = "Figure 13: Surveyee's opinion on government's budget on assisting the poor")
```
...
reduced_data %>%
  ...
  labs(x = element_blank(),
       ...
       caption = "Figure 14: Surveyee's opinion on government's budget on assisting other countries")
```
